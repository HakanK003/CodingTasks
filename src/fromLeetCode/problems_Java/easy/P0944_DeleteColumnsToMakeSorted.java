package fromLeetCode.problems_Java.easy;

/*
    Related Topics
    -> Array
    -> String

    Problem
    -> https://leetcode.com/problems/delete-columns-to-make-sorted/

        You are given an array of n strings strs, all of the same length.
        The strings can be arranged such that there is one on each line,
        making a grid. For example, strs = ["abc", "bce", "cae"] can be arranged as:
            abc
            bce
            cae
        You want to delete the columns that are not sorted lexicographically. In the above example (0-indexed),
        columns 0 ('a', 'b', 'c') and 2 ('c', 'e', 'e') are sorted while column 1 ('b', 'c', 'a') is not,
        so you would delete column 1.
        Return the number of columns that you will delete.


    Examples
    -> Example 1
            Input: strs = ["cba","daf","ghi"]
            Output: 1
            Explanation: The grid looks as follows:
              cba
              daf
              ghi
            Columns 0 and 2 are sorted, but column 1 is not, so you only need to delete 1 column.
    -> Example 2
            Input: strs = ["a","b"]
            Output: 0
            Explanation: The grid looks as follows:
              a
              b
            Column 0 is the only column and is sorted, so you will not delete any columns.
    -> Example 3
            Input: strs = ["zyx","wvu","tsr"]
            Output: 3
            Explanation: The grid looks as follows:
              zyx
              wvu
              tsr
            All 3 columns are not sorted, so you will delete all 3.


    Constraints
    -> n == strs.length
    -> 1 <= n <= 100
    -> 1 <= strs[i].length <= 1000
    -> strs[i] consists of lowercase English letters.
 */


/**
 * Approach 1 ->
 *
 *
 * Time Complexity -> O(n^2)
 *
 *
 * Space Complexity -> 2nd O(1)
 *
 *
 * Result
 *   First
 *   Runtime 21 ms Beats 26.7% | Memory 48 MB Beats 31.46%
 *   Second
 *   Runtime 12 ms Beats 59.33% | Memory 47.7 MB Beats 45.62%
 */
class Solution0944_1_1 {
    public int minDeletionSize(String[] strs) {

        int result = 0;
        char minDefault = 96;

        for (int i = 0; i < strs.length; i++) {
            for (int j = 0; j < strs.length; j++) {
                if (minDefault >= strs[j].charAt(i)) {
                    minDefault = strs[j].charAt(i);
                }else{
                    result++;
                    break;
                }
            }
        }

        return result;
    }
}

class Solution0944_1_2 {
    public int minDeletionSize(String[] strs) {

        int result = 0;

        for (int i = 0; i < strs[0].length(); i++) {
            for (int j = 1; j < strs.length; j++) {
                if (!(strs[j - 1].charAt(i) <= strs[j].charAt(i))) {
                    result++;
                    break;
                }
            }
        }
        return result;
    }
}



//4ms sample solution

//class Solution {
//    public int minDeletionSize(String[] A) {
//        int deleted = 0;
//        if (A == null || A.length == 0) return deleted;
//        int rowSize = A[0].length();
//
//        for (int col = 0; col < rowSize; col++){
//            if (!isSorted(A, col)) deleted++;
//        }
//
//        return deleted;
//    }
//
//    boolean isSorted(String[] A, int col){
//        int N = A.length;
//        char prev = (char)0;
//        for (int i = 0; i < N; i++){
//            char c = A[i].charAt(col);
//            //If out of order
//            if (c < prev) return false;
//            //update prev
//            prev = c;
//        }
//
//        return true; //In order the whole time
//    }
//}